...
const libSaml = () => {
  ...
  function getSigningScheme(
    sigAlg?: string
  ): SigningScheme | SigningSchemeHash | AdvancedSigningScheme {
    if (sigAlg) {
  ...
  return {
     ...
     */
    replaceTagsByValue(rawXML: string, tagValues: any): string {
      Object.keys(tagValues).forEach((t: string) => {
        rawXML = rawXML.replace(
          new RegExp(`{${t}}`, "g"),
          typeof tagValues[t] === "undefined"
            ? ""
            : tagValues[t]
        );
      });
    ...
    verifySignature(xml: string, opts?: SignatureVerifierOptions) {
      ...
      selection.forEach((signatureNode: Node) => {
        ...
        if (opts && opts.metadata) {
          ...
          if (certificateNode.length !== 0) {
            ...
            const x509Certificate = utility.normalizeCerString(
              x509CertificateData
            );
          ...
          },
          {
            ["ds:KeyInfo"]: [
              ...
              },
              {
                ["ds:X509Data"]: [
                  {
                    "ds:X509Certificate": utility.normalizeCerString(
                      certString
                    ),
                  },
                ],
