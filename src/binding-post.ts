 ...
 * @param idpInit
 */
async function base64LoginResponse({requestInfo = {}, entity, user = {}, customTagReplacement, encryptThenSign = false, AttributeStatement = [], idpInit}: Base64LoginResponseParams): Promise<BindingContext> {
    const idpSetting = entity.idp.entitySetting;
    const spSetting = entity.sp.entitySetting;
    // @ts-ignore
    const raw: string | undefined = idpSetting.generateID();
    const id: string = (typeof raw === 'string' ? raw : null) ?? `_${randomUUID()}`;
    const metadata = {
    ...
    if (metadata && metadata.idp && metadata.sp) {
        ...
        // @ts-ignore
        const sessionID: string = idpSetting?.generateID() ?? `${randomUUID()}`;
        ...
        // @ts-ignore
        const AssertionID: string = idpSetting?.generateID() ?? `_${randomUUID()}`;
        const tvalue: any = {
            ID: id,
