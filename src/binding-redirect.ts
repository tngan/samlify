...
import IdpProvider from './entity-idp';
import SpProvider from './entity-sp';
...
function buildRedirectURL(type: string, isSigned: boolean, rawSamlRequest: string, entitySetting: any, relayState: string = ''): string {
  ...
  if (isSigned) {
    ...
    return pvPair(type, octetString, true) + pvPair(urlParams.signature, encodeURIComponent(libsaml.constructMessageSignature(type + '=' + octetString, entitySetting.privateKeyFile, entitySetting.privateKeyFilePass, null, entitySetting.requestSignatureAlgorithm)));
  }
  return pvPair(type, samlRequest + relayState, true);
}
/**
...
*/
function loginRequestRedirectURL(entity: { idp: IdpProvider, sp: SpProvider }, rcallback?: (template: string) => string): string {
  let metadata: any = {
