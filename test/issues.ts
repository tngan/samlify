...
test("#31 query param for sso/slo is SamlRequest", (t: ExecutionContext) => {
...
test("#31 query param for sso/slo is SamlResponse", (t: ExecutionContext) => {
...
test("#31 query param for sso/slo returns error", (t: ExecutionContext) => {
  try {
...
(() => {
  ...
  test("#33 sp metadata acs index should be increased by 1", (t: ExecutionContext) => {
  ...
  test("#352 no index attribute for sp SingleLogoutService nodes", (t: ExecutionContext) => {
  ...
  test("#352 no index attribute for idp SingleSignOnService nodes", (t: ExecutionContext) => {
  ...
  test("#352 no index attribute for idp SingleLogoutService nodes", (t: ExecutionContext) => {
  ...
  test("#86 duplicate issuer throws error", (t: ExecutionContext) => {
  ...
  test("#87 add existence check for signature verification", (t: ExecutionContext) => {
    try {
      libsaml.verifySignature(
  ...
  test("#91 idp gets single sign on service from the metadata", (t: ExecutionContext) => {
    t.is(idp.entityMeta.getSingleSignOnService("post"), "idp.example.com/sso");
  });

  test("#98 undefined AssertionConsumerServiceURL with redirect request", (t: ExecutionContext) => {
    ...
    const authnRequest: any = select(
      xml,
      "/*[local-name(.)='AuthnRequest']"
    )[0];
    const index = Object.keys(authnRequest.attributes).find(
      (i: string) =>
        authnRequest.attributes[i].nodeName === "AssertionConsumerServiceURL"
    ) as any;
    t.is(
      authnRequest.attributes[index].nodeValue,
