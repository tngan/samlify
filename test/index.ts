...
(() => {
  ...
  test('sign a SAML message with RSA-SHA1', () => {
    // RSA-SHA1 must be requested explicitly — the default scheme is RSA-SHA256
    // (`xmldsig-core §6.4`, `saml-sec-consider §6.5`).
    expect(libsaml.constructMessageSignature(octetString, _spPrivPem, _spPrivKeyPass, undefined, signatureAlgorithms.RSA_SHA1).toString('base64')).toBe(signatureB64SHA1);
  });
