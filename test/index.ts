...
(() => {
  ...
  test('verify a XML signature signed by RSA-SHA256 with metadata', t => {
    t.is(libsaml.verifySignature(_decodedRequestSHA256, _decodedRequestSignatureSHA256, { cert: SPMetadata, signatureAlgorithm: signatureAlgorithms.RSA_SHA256 }), true);
  });
  test('verify a XML signature signed by RSA-SHA512 with metadata', t => {
    t.is(libsaml.verifySignature(_decodedRequestSHA512, _decodedRequestSignatureSHA512, { cert: SPMetadata, signatureAlgorithm: signatureAlgorithms.RSA_SHA512 }), true);
  });
