...
test("create login request with redirect binding using default template and parse it", async (t: ExecutionContext) => {
  ...
  t.is(
    extract.nameIDPolicy.format,
    "urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress"
  );
  t.is(extract.nameIDPolicy.allowCreate, "false");
});

test("create login request with post binding using default template and parse it", async (t: ExecutionContext) => {
  const {
    relayState,
    type,
    entityEndpoint,
...
test("signed in sp is not matched with the signed notation in idp with post request", (t: ExecutionContext) => {
...
test("signed in sp is not matched with the signed notation in idp with redirect request", (t: ExecutionContext) => {
  const _idp = identityProvider({
    ...defaultIdpConfig,
    metadata: noSignedIdpMetadata,
  });
...
test("create login request with redirect binding using [custom template]", (t: ExecutionContext) => {
...
test("create login request with post binding using [custom template]", (t: ExecutionContext) => {
  const _sp = serviceProvider({
    ...defaultSpConfig,
    loginRequestTemplate: {
      context:
...
test("create login response with undefined binding", async (t: ExecutionContext) => {
...
test("create post login response", async (t: ExecutionContext) => {
  const user = { email: "user@esaml2.com" };
...
test("create logout request with redirect binding", (t: ExecutionContext) => {
...
test("create logout request with post binding", (t: ExecutionContext) => {
  const {
    relayState,
    type,
    entityEndpoint,
...
test("create logout request when idp only has one binding", (t: ExecutionContext) => {
...
test("create logout response with undefined binding", (t: ExecutionContext) => {
  try {
...
test("create logout response with redirect binding", (t: ExecutionContext) => {
  const { id, context } = idp.createLogoutResponse(
    sp,
    {},
    "redirect",
...
test("create logout response with post binding", (t: ExecutionContext) => {
  const {
    relayState,
    type,
    entityEndpoint,
...
test("send response with signed assertion and parse it", async (t: ExecutionContext) => {
...
test("send response with signed assertion + custom transformation algorithms and parse it", async (t: ExecutionContext) => {
  // sender (caution: only use metadata and public key when declare pair-up in oppoent entity)
...
test("send response with [custom template] signed assertion and parse it", async (t: ExecutionContext) => {
  // sender (caution: only use metadata and public key when declare pair-up in oppoent entity)
...
test("send response with signed message and parse it", async (t: ExecutionContext) => {
  // sender (caution: only use metadata and public key when declare pair-up in oppoent entity)
  const user = { email: "user@esaml2.com" };
  const { id, context: SAMLResponse } = await idpNoEncrypt.createLoginResponse(
    spNoAssertSign,
...
test("send response with [custom template] and signed message and parse it", async (t: ExecutionContext) => {
  // sender (caution: only use metadata and public key when declare pair-up in oppoent entity)
...
test("send login response with signed assertion + signed message and parse it", async (t: ExecutionContext) => {
...
test("send login response with [custom template] and signed assertion + signed message and parse it", async (t: ExecutionContext) => {
  const spWantMessageSign = serviceProvider({
    ...defaultSpConfig,
    wantMessageSigned: true,
  });
...
test("send login response with encrypted non-signed assertion and parse it", async (t: ExecutionContext) => {
  const user = { email: "user@esaml2.com" };
...
test("send login response with encrypted signed assertion and parse it", async (t: ExecutionContext) => {
  const user = { email: "user@esaml2.com" };
  const { id, context: SAMLResponse } = await idp.createLoginResponse(
    sp,
    sampleRequestInfo,
...
test("send login response with [custom template] and encrypted signed assertion and parse it", async (t: ExecutionContext) => {
  const user = { email: "user@esaml2.com" };
...
test("send login response with encrypted signed assertion + signed message and parse it", async (t: ExecutionContext) => {
  const spWantMessageSign = serviceProvider({
    ...defaultSpConfig,
    wantMessageSigned: true,
  });
...
test("send login response with [custom template] encrypted signed assertion + signed message and parse it", async (t: ExecutionContext) => {
  const spWantMessageSign = serviceProvider({
    ...defaultSpConfig,
    wantMessageSigned: true,
  });
...
test("idp sends a redirect logout request without signature and sp parses it", async (t: ExecutionContext) => {
...
test("idp sends a redirect logout request with signature and sp parses it", async (t: ExecutionContext) => {
...
test("idp sends a post logout request without signature and sp parses it", async (t: ExecutionContext) => {
  const {
    relayState,
    type,
    entityEndpoint,
    id,
    context,
  } = idp.createLogoutRequest(sp, "post", {
    logoutNameID: "user@esaml2.com",
  }) as PostBindingContext;
  t.is(typeof id, "string");
  t.is(typeof context, "string");
...
test("idp sends a post logout request with signature and sp parses it", async (t: ExecutionContext) => {
  const {
    relayState,
    type,
    entityEndpoint,
    id,
    context,
  } = idp.createLogoutRequest(spWantLogoutReqSign, "post", {
    logoutNameID: "user@esaml2.com",
  }) as PostBindingContext;
  t.is(typeof id, "string");
  t.is(typeof context, "string");
...
// simulate init-slo
test("sp sends a post logout response without signature and parse", async (t: ExecutionContext) => {
  ...
  const { samlContent, extract } = await idp.parseLogoutResponse(sp, "post", {
    body: { SAMLResponse },
  });
  t.is(extract.signature, null);
  t.is(extract.issuer, "https://sp.example.org/metadata");
...
test("sp sends a post logout response with signature and parse", async (t: ExecutionContext) => {
  const {
    relayState,
    type,
    entityEndpoint,
...
test("send login response with encrypted non-signed assertion with EncryptThenSign and parse it", async (t: ExecutionContext) => {
  const user = { email: "user@esaml2.com" };
  const {
    id,
    context: SAMLResponse,
...
test("Customize prefix (saml2) for encrypted assertion tag", async (t: ExecutionContext) => {
...
test("Customize prefix (default is saml) for encrypted assertion tag", async (t: ExecutionContext) => {
  const user = { email: "test@email.com" };
  const { id, context: SAMLResponse } = await idp.createLoginResponse(
    sp,
    sampleRequestInfo,
...
test("avoid malformatted response", async (t: ExecutionContext) => {
  // sender (caution: only use metadata and public key when declare pair-up in oppoent entity)
...
test("should reject signature wrapped response - case 1", async (t: ExecutionContext) => {
  //
  const user = { email: "user@esaml2.com" };
  const { id, context: SAMLResponse } = await idpNoEncrypt.createLoginResponse(
    sp,
...
test("should reject signature wrapped response - case 2", async (t: ExecutionContext) => {
  //
  const user = { email: "user@esaml2.com" };
  const { id, context: SAMLResponse } = await idpNoEncrypt.createLoginResponse(
    sp,
  ...
  const wrappedResponse = Buffer.from(xmlWrapped).toString("base64");
  try {
    const result = await sp.parseLoginResponse(idpNoEncrypt, "post", {
      body: { SAMLResponse: wrappedResponse },
    });
  } catch (e) {
    t.is(e.message, "ERR_POTENTIAL_WRAPPING_ATTACK");
...
test("should throw two-tiers code error when the response does not return success status", async (t: ExecutionContext) => {
  try {
    const _result = await sp.parseLoginResponse(idpNoEncrypt, "post", {
