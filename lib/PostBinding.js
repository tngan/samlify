...
xmlTag = require('./urn').tags.xmlTag,
...
var PostBinding = function PostBinding(){
    return {
        ...
        base64LoginRequest: function base64LoginRequest(referenceTagXPath,entity,rcallback){
            ...
            if(metadata && metadata.idp && metadata.sp) {
                ...
                if(metadata.idp.isWantAuthnRequestsSigned()) {
                    return SamlLib.constructSAMLSignature(rawSamlRequest,referenceTagXPath,metadata.sp.getX509Certificate('signing'),spSetting.privateKeyFile,spSetting.privateKeyFilePass); // has problem
                } else {
         ...
         * @param  {function} rcallback     used when developers have their own login response template
         */
        base64LoginResponse: function base64LoginResponse(requestInfo,referenceTagXPath,entity,user,rcallback){
            var metadata = {
                idp: entity.idp.entityMeta,
                sp: entity.sp.entityMeta
            },
            ...
            if(metadata && metadata.idp && metadata.sp) {
                ...
                if(idpSetting.loginResponseTemplate){
                    ...
                    fiveMinutesLater = new Date(now.getTime());
                    fiveMinutesLater.setMinutes(fiveMinutesLater.getMinutes()+5);
                    fiveMinutesLater = new Date(fiveMinutesLater).toISOString();
                    now = now.toISOString();
                    var tvalue = {
                ...
                if(metadata.sp.isWantAssertionsSigned()) {
                    return SamlLib.constructSAMLSignature(rawSamlResponse,referenceTagXPath,metadata.idp.getX509Certificate('signing'),idpSetting.privateKeyFile,idpSetting.privateKeyFilePass); // has problem
                } else {
                    // No need to embeded XML signature
        ...
        base64LogoutRequest: function base64LogoutRequest(user,referenceTagXPath,entity,relayState,rcallback){
            ...
            if(metadata && metadata.init && metadata.target) {
                ...
                if(initSetting.loginRequestTemplate){
                    ...
                    rawSamlRequest = SamlLib.replaceTagsByValue(SamlLib.defaultLogoutRequestTemplate,tvalue);
                }
                if(entity.target.entitySetting.wantLogoutRequestSigned) {
                    // Need to embeded XML signature
                    return SamlLib.constructSAMLSignature(rawSamlRequest,referenceTagXPath,metadata.sp.getX509Certificate('signing'),initSetting.privateKeyFile,initSetting.privateKeyFilePass);
                } else {
                    // No need to embeded XML signature
                    return Utility.base64Encode(rawSamlRequest);
                }
            ...
            if(metadata && metadata.init && metadata.target) {
                ...
                if(initSetting.logoutResponseTemplate){
                    ...
                    if(entity.target.entitySetting.wantLogoutResponseSigned) {
                        // Need to embeded XML signature
                        return SamlLib.constructSAMLSignature(rawSamlResponse,referenceTagXPath,metadata.idp.getX509Certificate('signing'),initSetting.privateKeyFile,initSetting.privateKeyFilePass);
                    } else {
                        // No need to embeded XML signature
                        return Utility.base64Encode(rawSamlResponse);
                    }
