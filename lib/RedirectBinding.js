...
var RedirectBinding = function RedirectBinding() {
    ...
    */
    var buildRedirectURL = function buildRedirectURL(type, isSigned, rawSamlRequest, entitySetting, relayState) {
        ...
        var queryParam = SamlLib.getQueryParamByType(type);
        if (isSigned) {
            ...
            return pvPair(queryParam, octetString, true) + pvPair(urlParams.signature, encodeURIComponent(SamlLib.constructMessageSignature(type + '=' + octetString, entitySetting.privateKeyFile, entitySetting.privateKeyFilePass, null, entitySetting.requestSignatureAlgorithm)));
        } else {
            return pvPair(queryParam, samlRequest + _relayState, true);
        }
    };
