...
pki = forge.pki,
algorithms = require('./urn').wording.algorithms,
digest = require('./urn').wording.digest,
fs = require('fs'),
xml = require('xml'),
xpath = require('xpath');
...
var SamlLib = function SamlLib(){
    ...
    *
    */
    var getSigningScheme = function(key){
        var algAlias = nrsaAlgMapping[key];
        if (algAlias !== undefined) {
            return algAlias;
        } else {
            return nrsaAlgMapping[algorithms.RSA_SHA1]; // default value
        }
    };

    var getDigestMethod = function(key){
        var digAlias = digest[key];
        if (digAlias !== undefined) {
            return digAlias;
        } else {
    ...
    */
    ...
    return {
        ...
        */
        constructSAMLSignature: function constructSAMLSignature(xmlString,referenceXPath,x509,keyFile,passphrase,signatureAlgorithm){
            var sig = new SignedXml(),
            pem;
            // Add assertion sections as reference
            if(referenceXPath&&referenceXPath!==''){
                sig.addReference(referenceXPath,null,getDigestMethod(signatureAlgorithm));
            }
        ...
        */
        verifySignature: function verifySignature(xml,signature,opts){
            ...
            signatureAlgorithm = options.signatureAlgorithm || algorithms.RSA_SHA1, // SS1.1
            sig = new SignedXml();
        ...
        */
        verifyMessageSignature: function verifyMessageSignature(metadata,octetString,signature,verifyAlgorithm){
            ...
            key = new nrsa(pem,{
                signingScheme: getSigningScheme(verifyAlgorithm) // SS-1.1
            });
