...
pki = forge.pki,
algorithms = urn.algorithms,
signatureAlgorithms = algorithms.signature,
digestAlgorithms = algorithms.digest,
certUsage = urn.wording.certUse,
fs = require('fs'),
xml = require('xml'),
    ...
    return {
        ...
        */
        verifySignature: function verifySignature(xml,signature,opts){
            ...
            signatureAlgorithm = options.signatureAlgorithm || signatureAlgorithms.RSA_SHA1, // SS1.1
            sig = new SignedXml();
        ...
        */
        encryptAssertion: function encryptAssertion(sourceEntity,targetEntity,entireXML,callback){
            ...
            console.log('flag1',entireXML);
            if(entireXML){
                var sourceEntitySetting = sourceEntity.entitySetting,
                targetEntitySetting = targetEntity.entitySetting,
                sourceEntityMetadata = sourceEntity.entityMeta,
                targetEntityMetadata = targetEntity.entityMeta,
                assertionNode = getEntireBody(new dom().parseFromString(entireXML),'Assertion'),
                assertion = assertionNode !== undefined ? Utility.parseString(assertionNode.toString()) : '';
        ...
        decryptAssertion: function decryptAssertion(type, here, from, entireXML, callback){
            ...
            if(entireXML){
                ...
                if(type === 'SAMLResponse' && from.entitySetting.isAssertionEncrypted){
                    ...
                    encryptedData = encryptedDataNode !== undefined ? Utility.parseString(encryptedDataNode.toString()) : '';
