...
algorithms = require('./urn').algorithms,
signatureAlgorithms = algorithms.signature,
dataEncryptionAlgorithm = algorithms.encryption.data, // SS1.1
keyEncryptionAlgorithm = algorithms.encryption.key, // SS1.1
...
module.exports = function (entitySetting,entityMetaClass,entityMeta) {
    ...
    */
    Entity.prototype.abstractBindingParser = function abstractBindingParser(opts,binding,req,targetEntityMetadata,parseCallback) {
        ...
        _here = this, //SS-1.1 (refractor later on)
        _from = options.from, // SS-1.1 (refractor later on)
        _checkSignature = options.checkSignature === true,
        _decryptAssertion = options.decryptAssertion === true,
        ...
        if (binding == bindDict.redirect && _supportBindings.indexOf(nsBinding[binding]) !== -1) {
            ...
            if (_checkSignature) {
                // verify the signature
                if (!SamlLib.verifySignature(_decodedRequest,_parseResult.extract.signature,{
                    cert:targetEntityMetadata,
                    signatureAlgorithm:this.entitySetting.requestSignatureAlgorithm
                })) {
                    throw new Error('Incorrect signature');
                }
            }
