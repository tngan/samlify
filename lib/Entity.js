    ...
    Entity.prototype.abstractBindingParser = function abstractBindingParser(opts, binding, req, targetEntityMetadata, parseCallback) {
        ...
        if (_actionType == 'login') {
            if (_entityMeta.getAssertionConsumerService) {
                var _assertionConsumerService = _entityMeta.getAssertionConsumerService(binding);
                if (!_assertionConsumerService) {
                    _supportBindings = [];
                }
            } else if (_entityMeta.getSingleSignOnService) {
                var _singleSignOnService = _entityMeta.getSingleSignOnService(binding);
                if (!_singleSignOnService) {
                    _supportBindings = [];
                }
            }
        } else if (_actionType == 'logout') {
            var _singleLogoutServices = _entityMeta.getSingleLogoutService(binding);
            if (!_singleLogoutServices) {
                _supportBindings = [];
            }
        }
        ...
        if (binding == bindDict.redirect && _supportBindings.indexOf(nsBinding[binding]) !== -1) {
            ...
            var samlContent = reqQuery[SamlLib.getQueryParamByType(_parserType)];
            ...
            var _encodedRequest = req.body[SamlLib.getQueryParamByType(_parserType)];
